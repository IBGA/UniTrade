...
Then the item posting of item with name "<item_name>", item description "<item_description>", item image "<item_image>", item condition "<item_condition>", item price "<item_price>", item quantity "<item_quantity>", item category "<item_category>", is created and tagged with university "<existing_university>" and course "<existing_course>"
    Examples:
...
Then a "Fields filled incorrectly" error message is issued and the fields "<incorrectly_filled_fields>" are highlighted in red
    Examples:
        | existing_university | existing_course | incorrectly_filled_fields |
        | University of Toronto | CSC108 | item name |
        | University of Toronto | CSC108 | item description, item condition, item quantity |
